...
const searchSimilarByFile = async (fileInfo) => {
  ...
  try {
    loading.value = true
    const mediaResponse = await fetch(`/media/${fileInfo.id}`)
    if (!mediaResponse.ok) {
    ...
    const response = await searchByImage(queryFile, null, 30)
    const token = Date.now().toString()
    stashImageSearchResult(token, response.files || [])
    router.push({ name: 'home', query: { img_search: token } })
    ElMessage.success(`找到 ${(response.files || []).length} 个相似结果`)
  } catch (error) {
    ElMessage.error('查相似失败')
    console.error('Error searching similar image from favorites:', error)
  } finally {
    loading.value = false
  }
}
...
onActivated(async () => {
  ...
  if (actived.value) {
    try {
      loading.value = true
      const response = await getFavoritesList(0, 1)
