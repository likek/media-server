...
let lastRegisteredIv = Symbol('unset')

const resolveIvValue = () => {
  const routeIv = Array.isArray(route.query.iv) ? route.query.iv[0] : route.query.iv
  const locationIv = typeof window !== 'undefined' ? new URLSearchParams(window.location.search).get('iv') : null
  const rawValue = routeIv ?? locationIv
...
const registerCurrentUser = async (iv) => {
  const normalizedIv = iv ?? null
  if (lastRegisteredIv === normalizedIv) {
    return
  }
  await registerUser(normalizedIv)
  lastRegisteredIv = normalizedIv
...
onMounted(() => {
  ...
  registerCurrentUser(resolveIvValue())
    .then(() => {
    })
    .catch((error) => {
...
watch(() => route.query.iv, async () => {
  try {
    await registerCurrentUser(resolveIvValue())
  } catch (error) {
    console.error('iv变更后自动注册失败:', error)
  }
})
</script>
