...
const encodeWsMessage = (message) => {
  const salt = fingerprintInfo?.salt
  if (!salt) return message
  const encrypted = aesEncrypt(JSON.stringify(message), salt)
  return {
    [pickEnvelopeKey()]: encrypted
  }
}
const decodeWsMessage = (rawMessage) => {
  const parsed = JSON.parse(rawMessage)
  const encryptedPayload = getEnvelopePayload(parsed)
  if (!encryptedPayload) {
    return parsed
  }
  const salt = fingerprintInfo?.salt
  if (!salt) {
    throw new Error('缺少WebSocket解密salt')
  }
...
export function connectWebSocket() {
  ...
  getEncryptedFingerprint().then(info => {
    ...
    ws.onmessage = function(event) {
      let data = {}
      try {
        data = decodeWsMessage(event.data)
      } catch (e) {
      ...
      switch (data.event) {
        ...
        case 'realtimeStatusUpdated':
          window.dispatchEvent(new CustomEvent('ws-realtime-status', {
            detail: data.data || {}
          }))
          break
...
export function sendWebSocketMessage(message) {
  ...
  try {
    ws.send(JSON.stringify(encodeWsMessage(message)))
    return true
  } catch (error) {
