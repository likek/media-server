...
async function captureVideoFrame(videoPath, outputPath, time = "80%", resizeHeight = null) {
  assertNoNullByteInPath(videoPath);
  assertNoNullByteInPath(outputPath);
  ...
  const assertOutput = () => {
    if (!fs.existsSync(outputPath)) {
      throw new Error("output frame file not created");
    }
    const stat = fs.statSync(outputPath);
    if (!stat.isFile() || stat.size <= 0) {
      throw new Error("output frame file empty");
    }
  };
  ...
  const captureByScreenshots = async (timestamps) => {
    ...
    if (resizeHeight) {
      screenshotOptions.size = `?x${resizeHeight}`;
    }
  ...
  const captureFirstFrame = async () => {
    ...
    if (resizeHeight) {
      outputOptions.splice(1, 0, `-vf scale=-1:${resizeHeight}`);
    }
    return new Promise((resolve, reject) => {
      ffmpeg(videoPath)
        .output(outputPath)
...
async function generateThumbnail(videoPath, thumbnailPath, time = "80%") {
  return captureVideoFrame(videoPath, thumbnailPath, time, 240);
}

// 导出视频原始分辨率帧
async function generateVideoFrame(videoPath, outputPath, time = "80%") {
  return captureVideoFrame(videoPath, outputPath, time);
}
