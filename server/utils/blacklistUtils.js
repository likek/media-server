...
function getBlacklistExpiresAtByAddedTime(addedTime) {
  if (!addedTime) {
    return null;
  }
  const timestamp = new Date(addedTime).getTime();
  if (Number.isNaN(timestamp)) {
    return null;
  }
  return new Date(timestamp + blacklistDurationMs).toISOString();
}
...
function addToBlacklist(req, userId) {
  try {
    ...
    if (inBlacklist) {
      // 如果已经在黑名单中，不做操作
      return { success: true, error: null, timeLeft: getBlackListTimeLeftByAddedTime(row?.added_time) };
    } else {
      // 插入新的记录
