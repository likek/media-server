...
function checkAndRenderInitialFiles() {
  while (
    mediaContainer.scrollHeight <=
      window.innerHeight - topContainer.offsetHeight &&
    renderedFilesCount < totalFiles.length
  ) {
    renderMoreFiles();
  }
}
...
function renderFiles(files) {
  ...
  files.forEach((file) => {
    ...
    if (file.type !== "folder") {
      ...
      fileInfoElement.innerHTML += `<span>修改日期: ${new Date(
        file.lastModified
      ).toLocaleString()}</span>`;
      fileInfoElement.style =
        "font-size: 10px; color: #666;margin: 0 0 4px 0;display:flex;justify-content: space-between";
...
function renderMoreFiles() {
  const nextFiles = totalFiles.slice(
    renderedFilesCount,
    renderedFilesCount + pageSize
  );
  if (nextFiles.length) {
    console.log(
      `render: ${renderedFilesCount}, ${renderedFilesCount + pageSize}`
    );
...
mediaContainer.addEventListener("scroll", () => {
  if (
    mediaContainer.clientHeight + mediaContainer.scrollTop >=
    mediaContainer.scrollHeight - 50
  ) {
    renderMoreFiles();
  }
});
...
function updateProgressBar(loaded, total) {
  ...
  progressBarValue.innerHTML = `${(loaded / 1024 / 1024).toFixed(2)}MB/${(
    total /
    1024 /
    1024
  ).toFixed(2)}MB`;
...
function openTxtModal(txt, filePath, nextStart, numLines) {
  ...
  const txtContentCb = (data) => {
    if (data.content) {
      ...
      currPage.innerText = Math.ceil(
        modalTxt.__nextStart / modalTxt.__numLines
      );
      if (data.isLastPage) {
  ...
  btnTxtModelNext.onclick = () => {
    getTxtContent(filePath, modalTxt.__nextStart, modalTxt.__numLines).then(
      txtContentCb
    );
  };
          ...
          if (currentPath === data.data.dirPath) {
            ...
            if (refresh) {
              loadMedia(data.data.dirPath)
                ...
                .catch((error) => {
                  showToast("父级更新失败", "warn");
                });
            }
          } else {
            // showToast(`文件夹${data.data.dirPath}发生了更新`);
          }
